...
export function PaginaAreasServicios() {
  ...
  useEffect(() => {
    ...
    listarAreas()
      ...
      .catch((errorCarga) => {
        if (paginaVigente) {
          establecerError(obtenerMensajeError(errorCarga, "No fue posible cargar las áreas y servicios."));
          establecerCargando(false);
        }
      });
    return () => {
      ...
      <section className="portal-seccion">
        <div className="portal-contenedor">
          ...
          {!cargando && areas.length > 0 ? (
            <div className="pagina-rejilla-tarjetas">
              {areas.map((area, indice) => (
                <article className="pagina-tarjeta" key={area.codigo}>
                  <img className="pagina-tarjeta-imagen" src={area.urlImagen} alt={area.nombre} />
            ...
            <p className="portal-estado-vacio">Aún no hay áreas con información e imagen completas para mostrar.</p>
          )}
        </div>
      </section>
    </>
